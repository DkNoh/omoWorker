...
class SmsHistoryControllerTest {
  ...
  void setUp() {
    mockMvc =
        MockMvcBuilders.standaloneSetup(new SmsHistoryController(service))
  ...
  void data는_정상_요청을_DTO에_매핑하고_ApiResponse로_응답한다() throws Exception {
    ...
    ArgumentCaptor<SmsHistorySearchRequestDTO> captor =
        ArgumentCaptor.forClass(SmsHistorySearchRequestDTO.class);
    ...
    assertThat(captor.getValue().getSendType()).isEqualTo("1");
    assertThat(captor.getValue().getSendStatus()).isEqualTo("1");
    assertThat(captor.getValue().getStartDt()).isEqualTo("1");
    assertThat(captor.getValue().getEndDt()).isEqualTo("1");
    assertThat(captor.getValue().getReceiverNo()).isEqualTo("1");
  }
  ...
  void data의_숫자_파라미터가_잘못되면_400이고_서비스를_호출하지_않는다() throws Exception {
    // when / then
    mockMvc
        .perform(get("/sms/history/data").param("page", "invalid"))
