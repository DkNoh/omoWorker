...
class AuditLogControllerTest {
  ...
  @BeforeEach
  void setUp() {
    mockMvc = MockMvcBuilders.standaloneSetup(new AuditLogController(service)).build();
  }
  ...
  void data는_ApiResponse_포맷으로_응답한다() throws Exception {
    ...
    given(service.search(any()))
        .willReturn(PageResponseDTO.of(List.of(), new AuditLogSearchRequestDTO(), 0));

    // when / then
    mockMvc
        .perform(get("/basic/audit-log/data"))
