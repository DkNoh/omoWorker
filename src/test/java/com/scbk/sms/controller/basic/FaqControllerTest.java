...
class FaqControllerTest {
  ...
  @Mock private FaqService service;
  ...
  void data는_ApiResponse_포맷으로_응답한다() throws Exception {
    ...
    given(service.search(any()))
        .willReturn(PageResponseDTO.of(List.of(), new FaqSearchRequestDTO(), 0));

    // when / then
    mockMvc
        .perform(get("/basic/faq/data"))
  ...
  void delete는_삭제_성공_메시지를_반환한다() throws Exception {
    // when / then
    mockMvc
        .perform(post("/basic/faq/delete").param("noticeId", "1"))
        .andExpect(status().isOk())
