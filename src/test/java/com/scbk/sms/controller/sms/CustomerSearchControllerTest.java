...
class CustomerSearchControllerTest {
  ...
  @Mock private CustomerSearchService service;

  private MockMvc mockMvc;

  @BeforeEach
  void setUp() {
    mockMvc = MockMvcBuilders.standaloneSetup(new CustomerSearchController(service)).build();
  }
  ...
  void data는_ApiResponse_포맷으로_응답한다() throws Exception {
    ...
    given(service.search(any()))
        .willReturn(PageResponseDTO.of(List.of(), new CustomerSearchSearchRequestDTO(), 0));

    // when / then
    mockMvc
        .perform(get("/sms/customer-search/data"))
  ...
  void create는_등록_성공_메시지를_반환한다() throws Exception {
    ...
    mockMvc
        .perform(
            post("/sms/customer-search/create")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"customerNm\":\"1\",\"mobileNo\":\"1\",\"email\":\"1\",\"birthDt\":\"1\",\"genderCd\":\"1\",\"agreeYn\":\"1\",\"useYn\":\"1\"}"))
        .andExpect(status().isOk())
  ...
  void update는_수정_성공_메시지를_반환한다() throws Exception {
    ...
    mockMvc
        .perform(
            post("/sms/customer-search/update")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"customerNm\":\"1\",\"mobileNo\":\"1\",\"email\":\"1\",\"birthDt\":\"1\",\"genderCd\":\"1\",\"agreeYn\":\"1\",\"useYn\":\"1\"}"))
        .andExpect(status().isOk())
  ...
  void delete는_삭제_성공_메시지를_반환한다() throws Exception {
    // when / then
    mockMvc
        .perform(post("/sms/customer-search/delete").param("customerId", "1"))
        .andExpect(status().isOk())
