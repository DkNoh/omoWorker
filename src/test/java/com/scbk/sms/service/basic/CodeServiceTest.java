...
class CodeServiceTest {
  ...
  @Mock private CodeMapper mapper;

  private CodeService service;

  @BeforeEach
  void setUp() {
    service = new CodeService(mapper);
  }

  @Test
  void 목록_조회는_페이지_응답으로_감싼다() {
    // given
    CodeSearchRequestDTO request = new CodeSearchRequestDTO();
    request.setPage(1);
    request.setSize(10);
    given(mapper.count(request)).willReturn(1);
    given(mapper.selectList(request)).willReturn(List.of(new CodeVO()));

    // when
    PageResponseDTO<CodeVO> result = service.search(request);
